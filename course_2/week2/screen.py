...
class Vec2d:
    ...
    def __add__(self, obj):
        return Vec2d((self.x + self.y, obj.x + obj.y))
...
def get_point(points, alpha, deg=None):
    ...
    if deg == 0:
        return points[0]
    a = (points[deg] * alpha)
    b = (get_point(points, alpha, deg - 1) * (1 - alpha))
    print(a + b)
    return a + b
...
def set_points(points, speeds):
    ...
    for p in range(len(points)):
        points[p] = Vec2d((points[p] + speeds[p]).int_pair())
...
def worker(pygame):    
    ...
    while working:
        ...
        draw_points(gameDisplay, points)
        draw_points(gameDisplay, get_knot(points, steps), "line", 3, color)
        if not pause:
